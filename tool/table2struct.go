...
func (t *Table2Struct) Run() error {
	...
	for tableRealName, item := range tableColumns {
		...
		// 添加 method 获取真实表名
		if t.realNameMethod != "" {
			structContent += fmt.Sprintf("func (this *%s) %s() string {\n",
		...
		template.Parse(strings.NewReader(modelTemplate))
...
func ColumnDefaultTag(i interface{}) string {
	if i == nil {
		return ""
	}
	val := gconv.String(i)
...
func (t *Table2Struct) getColumns(table ...string) (tableColumns map[string][]column, err error) {
	...
	for rows.Next() {
		...
		columnDefault := ColumnDefaultTag(col.ColumnDefault)
		if t.enableJsonTag {
			//col.Json = fmt.Sprintf("`json:\"%s\" %s:\"%s\"`", col.Json, t.config.TagKey, col.Json)
			col.Tag = fmt.Sprintf("`%s:\"column:%s%s\" json:\"%s\"`", t.tagKey, col.Tag, columnDefault, t.ToLowerFirst(col.ColumnName))
		} else {
			col.Tag = fmt.Sprintf("`%s:\"column:%s%s\"`", t.tagKey, col.Tag, columnDefault)
		}
...
func (t *Table2Struct) ToLowerFirst(str string) string {
